...
int CuSuiteDetails(CuSuite *testSuite, CuString *details)
{
    ...
    runCount = testSuite->count - skipCount;
    for (i = 0; i < testSuite->count; ++i) {
        CuTest *testCase = testSuite->list[i];
        if (testCase->failed) {
            failCount++;
            CuStringAppendFormat(details, "[CuTest Fail : %d] %s: %s\n",
                                 failCount, testCase->name, testCase->message);
        }
        if (testCase->skipped) {
            skipCount++;
            //CuStringAppendFormat(details, "[CuTest Skip : %d] %s\n",skipCount, testCase->name);
        }
    }
    CuStringAppend(details, "\n----- Test Suite Summary -----\n");
    CuStringAppendFormat(details, "Total Tests Run: %d\n", runCount);
    CuStringAppendFormat(details, "Total Tests Skipped: %d\n", skipCount);
    CuStringAppendFormat(details, "Total Tests Passed: %d\n",
                         testSuite->count - skipCount - failCount);
    CuStringAppendFormat(details, "Total Tests Failed: %d\n", failCount);

    if ((failCount > 0) && (runCount > 0)) {
        return -1;
    } else {
        return 0;
    }
